...
<form method="get" class="sectionbox">
  <input type="hidden" name="days" value="{{ d.window_days }}">
  <div style="margin-bottom:6px"><b>Ticket type:</b>
    {% for t in d.avail_types %}
    <label style="margin:0 10px 0 2px"><input type="checkbox" name="type" value="{{ t }}" {% if t in d.sel_types %}checked{% endif %}> {{ t }}</label>
    {% else %}<span class="muted">none</span>{% endfor %}
  </div>
  <div style="margin-bottom:6px"><b>Status:</b>
    {% for s in d.avail_statuses %}
    <label style="margin:0 10px 0 2px"><input type="checkbox" name="status" value="{{ s }}" {% if s in d.sel_statuses %}checked{% endif %}> {{ s }}</label>
    {% else %}<span class="muted">none</span>{% endfor %}
  </div>
  <div>
...
<div class="cards">
 <div class="card"><div class="n">{{ d.assigned }}</div><div class="l">Matching tickets</div></div>
...
<h2>Completed</h2>
<table><tr><th>Key</th><th>Summary</th><th>Type</th><th>Cycle</th></tr>
{% for i in d.completed_list %}
<tr><td><a href="{{ i.url }}" target="_blank">{{ i.key }}</a></td><td>{{ i.summary }}</td><td>{{ i.type }}</td><td>{{ fmt(i.timeline.cycle_days) }}</td></tr>
{% else %}<tr><td colspan="4" class="muted">None.</td></tr>{% endfor %}
</table>
<h2>Open</h2>
<table><tr><th>Key</th><th>Summary</th><th>Type</th><th>Status</th><th>Stage</th><th>Open age</th><th>Days in stage</th></tr>
{% for i in d.open_list %}
<tr><td><a href="{{ i.url }}" target="_blank">{{ i.key }}</a></td><td>{{ i.summary }}</td>
<td>{{ i.type }}</td><td>{{ i.status }}</td>
<td>{{ i.stage }}</td><td>{{ fmt(i.age_days) }}</td><td>{{ fmt(i.timeline.days_in_stage(i.stage)) }}</td></tr>
{% else %}<tr><td colspan="7" class="muted">None.</td></tr>{% endfor %}
</table>
"""


def _indiv_filters(args):
    """Parse the Individual Activity filter args shared by the page and CSV routes."""
    ...
    try:
        ...
        min_age = 0
    return days, types, statuses, min_age


@bp.route("/reports/individual/<person>")
def individual(person):
    days, types, statuses, min_age = _indiv_filters(request.args)
    d = R.individual_activity(dataset(), person, days, types=types,
                              statuses=statuses, min_open_age=min_age)
    return page(INDIV, d=d)
...
def individual_csv(person):
    ...
    w.writerow(["Filters — types:", ", ".join(d["sel_types"]) or "all",
                "statuses:", ", ".join(d["sel_statuses"]) or "all",
                "min open age (days):", d["min_open_age"]])
    w.writerow([])
    w.writerow(["Summary", "Value"])
    w.writerow(["Matching tickets", d["assigned"]])
    ...
    w.writerow(["Key", "Summary", "Type", "Cycle days", "URL"])
    for i in d["completed_list"]:
        w.writerow([i.key, i.summary, i.type, i.timeline.cycle_days, i.url])
    w.writerow([])
    w.writerow(["Open tickets"])
    w.writerow(["Key", "Summary", "Type", "Status", "Stage", "Open age (days)", "Days in stage", "URL"])
    for i in d["open_list"]:
        w.writerow([i.key, i.summary, i.type, i.status, i.stage, i.age_days,
                    i.timeline.days_in_stage(i.stage), i.url])
